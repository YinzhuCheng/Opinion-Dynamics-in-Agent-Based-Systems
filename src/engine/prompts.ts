...
- 负向：${negativeDesc}`;
  ...
  const innerStateGuidelines = `内在状态机制（[[STATE]] 必须包含以下结构）：
  1. 个人发言记忆摘要：3 句，概括最近几轮自己说过的核心观点或情绪；若为首轮，可按实际句数输出。
  2. 他人发言记忆摘要：3 句，提炼最近几轮至少两名 Agent 的触发点；若可引用的发言不足，可按实际句数输出。
  3. 当前滤镜描述：3~5 句，拆分“长期状态”（人格画像、MBTI、大五人格、价值观、沟通风格、初始观点/立场、记忆集合）与“短期波动”（情绪、生理状态、安全感、当前目标、对他人可靠性的判断），并说明哪些因最新刺激发生变化。
  - 记忆摘要需采用滑动窗口：距离当前轮次过久的记忆要被替换，以便纳入最新线索。
  - [[STATE]] 必须引用上一轮自己的 [[STATE]]、[[THINK]] 以及上一轮各个 Agent 的发言、信任度矩阵偏好，说明哪些因素保持或更新。`;
